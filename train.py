...
tf.flags.DEFINE_float(
    "regularization_coef", 0.5, "Hypermeter combining adversarial loss and regularization term when adv-training",
)
...
tf.flags.DEFINE_integer("max_iter", 20, "Maximum number of substitutions allowed.(default: 20)")
tf.flags.DEFINE_float(
    "distance_threshold", 0.5, "The maximum distance between two substitutions (default: 0.5)",
)
tf.flags.DEFINE_integer(
    "max_candidates", 20, "Maximum number of substitution candidates per word. (default: 20)",
)
...
def train(
    ...
    with tf.Graph().as_default():
        ...
        if FLAGS.nn_type == "textcnn":
            embeddings, _, predictions, logits = textcnn(x_batch, dropout_keep_prob, FLAGS.data)
        elif FLAGS.nn_type == "textrnn":
            embeddings, _, predictions, logits = textrnn(x_batch, dropout_keep_prob, FLAGS.data)
        elif FLAGS.nn_type == "textbirnn":
            embeddings, _, predictions, logits = textbirnn(x_batch, dropout_keep_prob, FLAGS.data)
        ...
        if FLAGS.train_type == "adv":
            ...
            if FLAGS.nn_type == "textcnn":
                _, _, predictions_adv, logits_adv = textcnn(x_adv, dropout_keep_prob, FLAGS.data, reuse=True)
            elif FLAGS.nn_type == "textrnn":
                _, _, predictions_adv, logits_adv = textrnn(x_adv, dropout_keep_prob, FLAGS.data, reuse=True)
            elif FLAGS.nn_type == "textbirnn":
                _, _, predictions_adv, logits_adv = textbirnn(x_adv, dropout_keep_prob, FLAGS.data, reuse=True)
            ...
            standard_loss = FLAGS.adv_sigma * loss_normal + (1 - FLAGS.adv_sigma) * loss_adversarial
            pair_logits_loss = tf.losses.mean_squared_error(
                logits_adv, logits, weights=FLAGS.regularization_coef, reduction=tf.losses.Reduction.MEAN,
            )
        ...
        with sess.as_default():
            ...
            for epoch in range(FLAGS.num_epochs):
                ...
                sess.run(
                    train_initializer, feed_dict={x: train_seqs, x_mask: train_seqs_mask, y: train_labels},
                )
                ...
                try:
                    while True:
                        (temp_train_acc_normal, temp_train_acc_adversarial, step, summary, _,) = sess.run(
                            [acc_normal, acc_adversarial, step_update, merged_summary, opt,]
                        )
                        train_writer.add_summary(summary, step)
                        total_train_acc_normal += temp_train_acc_normal * FLAGS.batch_size
                        total_train_acc_adversarial += temp_train_acc_adversarial * FLAGS.batch_size
                except tf.errors.OutOfRangeError:
                    pass
                # validate the model, drop_keep_rate = 1.0
                tf.assign(dropout_keep_prob, 1.0)
                sess.run(
                    test_initializer, feed_dict={x: test_seqs, x_mask: test_seqs_mask, y: test_labels},
                )
                total_test_acc = 0
                ...
                logging_str = (
                    ...
                    + "Epoch {}\n".format(str(epoch + 1))
                    + "---------------------------\n"
                    + "Training normal accuracy is {}\n".format(total_train_acc_normal / no_train_examples)
                    + "Training adversarial accuracy is {}\n".format(total_train_acc_adversarial / no_train_examples)
                    + "Validation accuracy is {}\n".format(total_test_acc / no_test_examples)
                    + "---------------------------\n"
                )
