...
def main(argv=None):
    ...
    if not os.path.isfile(
        os.path.join(FLAGS.data_dir, 'aux_files', 'org_dic_%s_%d.pkl' % (FLAGS.data, MAX_VOCAB_SIZE))
    ):
        ...
        with open(
            os.path.join(FLAGS.data_dir, 'aux_files', 'org_dic_%s_%d.pkl' % (FLAGS.data, MAX_VOCAB_SIZE)), 'wb'
        ) as f:
            pickle.dump(org_dic, f, protocol=4)
        with open(
            os.path.join(FLAGS.data_dir, 'aux_files', 'org_inv_dic_%s_%d.pkl' % (FLAGS.data, MAX_VOCAB_SIZE)), 'wb'
        ) as f:
            pickle.dump(org_inv_dic, f, protocol=4)
    else:
        print('org_dic & org_inv_dic already exist, load the dict...')
        with open(
            os.path.join(FLAGS.data_dir, 'aux_files', 'org_dic_%s_%d.pkl' % (FLAGS.data, MAX_VOCAB_SIZE)), 'rb'
        ) as f:
            org_dic = pickle.load(f)
        with open(
            os.path.join(FLAGS.data_dir, 'aux_files', 'org_inv_dic_%s_%d.pkl' % (FLAGS.data, MAX_VOCAB_SIZE)), 'rb'
        ) as f:
            org_inv_dic = pickle.load(f)

    # Calculate the distance matrix
    if not os.path.isfile(
        os.path.join(FLAGS.data_dir, 'aux_files', 'small_dist_counter_%s_%d.npy' % (FLAGS.data, MAX_VOCAB_SIZE))
    ):
        ...
        small_dist_mat = np.load(
            os.path.join(FLAGS.data_dir, 'aux_files', 'small_dist_counter_%s_%d.npy' % (FLAGS.data, MAX_VOCAB_SIZE))
        )

    if not os.path.isfile(
        os.path.join(FLAGS.data_dir, 'aux_files', 'embeddings_glove_%s_%d.npy' % (FLAGS.data, MAX_VOCAB_SIZE))
    ):
        print('embeddings glove not exists, creating...')
        glove_model = utils.loadGloveModel('glove.840B.300d.txt', data_dir=FLAGS.data_dir)
        glove_embeddings, _ = utils.create_embeddings_matrix(
            glove_model, org_dic, dataset=FLAGS.data, data_dir=FLAGS.data_dir
        )
