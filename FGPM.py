...
def FGPM(
    ...
    synonyms = tf.cast(find_synonym(xs_org, dist_mat, batch_size, word_max_len, threshold=dis_threshold), tf.int32,)
    ...
    def one_step_attack(adv_xs, modified_mask, ys, synonyms, i):
        embeddings, embedded_chars, predictions, logits = query(adv_xs, 1.0, dataset, reuse=True)
        ...
        unsuccessful_mask = tf.logical_and(
            tf.equal(predictions, ys), tf.less_equal(modified_ratio, max_perturbed_percent),
        )
        ...
        projection = tf.reduce_sum(tf.multiply(synonyms_embed - xs_embed, Jacobian), axis=-1)
        ...
        # Step 4: Subsitution.
        _, pos = tf.nn.top_k(tf.reduce_max(projection, axis=-1), k=grad_update_interval)
        serial = tf.tile(tf.expand_dims(tf.range(0, batch_size, 1), -1), [1, grad_update_interval])
        ...
        synonym_pos = tf.expand_dims(tf.gather_nd(tf.argmax(projection, axis=2), indices_m), -1)
        ...
        delta = tf.SparseTensor(indices_m, updates - tf.gather_nd(modified_mask, indices_m), [batch_size, word_max_len])
        modified_mask = modified_mask + tf.sparse_tensor_to_dense(delta, validate_indices=False)
        delta = tf.SparseTensor(synonym_indices_m, -synonym, [batch_size, word_max_len, sn])
